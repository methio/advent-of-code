...
fs.readFile('input.txt', 'utf8', (err, data) => {
  ...
  i.forEach( (instruction, index) => {
    ...
    instructions.push({
      ...
      "from": parseInt(reducedInstructions[1] -1, 10),
      "to": parseInt(reducedInstructions[2] -1, 10)
    });
  ...
  instructions.forEach( (instruction, index) => {
    ...
    for( let i = 1; i <= instruction.numToMove; i++){
      const toMove = stacks[instruction.from].shift();
      moved.unshift(toMove);      
    }
    // console.log(moved, '----------', stacks);

    moved.forEach(move=>stacks[instruction.to].unshift(move));
